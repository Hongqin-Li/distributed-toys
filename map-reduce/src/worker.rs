...
impl Worker {
    fn intermediate_path(&self, map_idx: usize, reduce_idx: usize) -> PathBuf {
        self.dir.join(format!("mr-{}-{}.json", map_idx, reduce_idx))
    }

    fn output_path(&self, reduce_idx: usize) -> PathBuf {
        self.dir.join(format!("mr-out-{}.json", reduce_idx))
    }
    pub async fn launch(&self) -> Result<(), std::io::Error> {
        ...
        let client_id = thread::current().id();
            ...
            if let Some(t) = resp.task {
                ...
                match t.task {
                    TaskType::Map => {
                        for fname in t.files.iter() {
                            ...
                            for (i, values) in cnt.iter() {
                                let path = self.intermediate_path(t.id, i.to_owned());
                    ...
                    TaskType::Reduce => {
                        ...
                        for i in 0..nmap {
                            // Reduce
                            let path = self.intermediate_path(i, t.id);
                        ...
                        // Output
                        let path = self.output_path(t.id);
